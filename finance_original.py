...
from sqlalchemy import Column, String, create_engine, Integer, DateTime, Text, BLOB
...
if __name__ == "__main__":
    ...
    class Original(Base):
        ...
        # 表的结构:
        original_id = Column(Integer(), primary_key=True, autoincrement=True)
        original_name = Column(String(255), index=True)
        original_date = Column(DateTime)
        original_link = Column(String(255))
    ...
    for tt in range(page):
        tt += 1
        print(tt)
        rep1 = requests.get("http://finance.sina.com.cn/roll/index.d.html?cid=221431&page="+str(tt))
        rep1.encoding = "UTF-8"
        ...
        for item1 in soup1.find_all(class_="list_009"):
            for subitem1 in item1.find_all("li"):
                ...
                # try:
                session = DBSession()
                new_comment = Original(original_name=str(n1), original_date=d1, original_link=a1, original_text=str1)
